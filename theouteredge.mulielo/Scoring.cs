...
namespace theouteredge.mulielo
{
  public static class Scoring
  {
    ...
    /// </summary>
    /// <param name="_base">_base for the exponential score function (> 1) a value of 1 
    /// returns an liner scoring method</param>
    /// <returns>
    /// A function that takes parameter n for number of players and returns an array of the 
    /// points to assign to each place(summing to 1)
    /// </returns>
    ...
    {
      var output = Enumerable.Range(1, n).Select(p => Math.Pow(_base, n - (double)p) - 1);
      var sum = output.Sum();

      var results = output.Select(x =>
      {
        if (x == 0)
          return x;
        else
