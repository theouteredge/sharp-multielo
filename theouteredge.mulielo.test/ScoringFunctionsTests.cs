namespace theouteredge.mulielo.test
{
    public class ScoringFunctionsTests
    {
            ...
            Enumerable.Range(3, 11)
                ...
                .ForEach(n =>
                {
                    ...
                    Assert.That(scores.Sum(), Is.EqualTo(1).Within(0.000000000000001),
                        () => $"liner scoring algorithm does not sum to 1 for n={n}: {Util.format(scores)}");

                    Assert.That(scores.Min(), Is.EqualTo(0),
                        () => $"linear score function does not have minimum score of 0 for n={n}: {Util.format(scores)}");

                    var diff = difference(scores);
                    for (var i = 0; i < diff.Count() - 1; i++)
                        Assert.That(diff[i] - diff[i + 1], Is.EqualTo(0).Within(0.000000000000001),
                            () => $"linear score function is not monotonically decreasing for n={n}: out by {diff[i] - diff[i + 1]} {Util.format(diff)}");
                });
              ...
              .ForEach(b =>
              {
                  Enumerable.Range(3, 11)
                    ...
                    .ForEach(n =>
                    {
                        ...
                        Assert.That(scores.Sum(), Is.EqualTo(1).Within(0.000000000000001),
                            () => $"exponential scoring algorithm does not sum to 1 for base={b} n={n}: {Util.format(scores)}");

                        Assert.That(scores.Min(), Is.EqualTo(0),
                            () => $"exponential score function does not have minimum score of 0 for base={b} n={n}: {Util.format(scores)}");

                        var diff = difference(scores);
                        for (var i = 0; i < diff.Count() - 1; i++)
                            Assert.That(diff[i] - diff[i + 1], Is.GreaterThan(0).Within(0.000000000000001),
                                () => $"exponential score function is not monotonically decreasing for n={n}: out by {diff[i] - diff[i + 1]} {Util.format(diff)}");
                        ...
                        for (var i = 0; i < diff_diffs.Count(); i++)
                            Assert.That(diff[i], Is.GreaterThan(0).Within(0.000000000000001),
                                () => $"exponential score function is not monotonically decreasing for n={n}: out by {diff[i]} {Util.format(diff)}");
                     });
              });
        }
        ...
        public void Assert_That_Exponential_Scoring_Works_Matching_A_Python_Run()
        {
            ...
            Assert.That(scores.Sum(), Is.EqualTo(1).Within(0.000000000000001),
              () => $"exponential scoring algorithm does not sum to 1 for base=2.973957279878059 n=3: {Util.format(scores)}");

            Assert.That(scores.Min(), Is.EqualTo(0),
              () => $"exponential score function does not have minimum score of 0 for base=2.973957279878059 n=3: {Util.format(scores)}");
        }
